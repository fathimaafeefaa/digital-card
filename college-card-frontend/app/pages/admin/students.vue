<template>
  <div>
    <h1 class="text-2xl font-bold text-eau-navy mb-6">Manage Students</h1>

    <!-- Search and Filter -->
    <div class="bg-white rounded-xl shadow p-4 mb-6 flex justify-between items-center flex-wrap gap-4">
      <input
        v-model="search"
          ...
          <tbody class="divide-y divide-gray-100">
            <tr v-for="s in filteredStudents" :key="s.id" class="hover:bg-gray-50">
              ...
              <td class="px-6 py-3 flex gap-2">
                <NuxtLink
                  :to="`/card/${s.id}`"
                  class="text-eau-navy hover:underline text-xs font-semibold"
                >
                  View Card
                </NuxtLink>
                <button
